...
def parsing_with_pedice(observation_graph):
    global_sequence = []
    for i, t, o in observation_graph:
        global_sequence.append((i.name, t.relevant_label, -1, o.name))
    return global_sequence


def diagnosi_from_silent_closure(silent_closure_subgraph, final_states):
    ...
    # # print("DICT", dict)
    #global_sequence = parsing_with_pedice(silent_closure)
    global_sequence = parsing_with_pedice(silent_closure_subgraph)
    for i, t, o in silent_closure_subgraph:
        for el in final_states:
            if el == i.name and (i.name, NULL_SMIB, -1, "NQ") not in global_sequence:
                global_sequence.append((i.name, NULL_SMIB, -1, "NQ"))
            elif el == o.name and (o.name, NULL_SMIB, -1, "NQ") not in global_sequence:
                global_sequence.append((o.name, NULL_SMIB, -1, "NQ"))
    ...
    while check_number_of_states(global_sequence) \
        ...
        global_sequence = create_series_from_graph(
            global_sequence, final_states)  # usa sta  non dict
        ...
        #     global_sequence, n0, nq, final_states)
...
def unite_series_with_pedice(series_sequence, stati_accettati):
    ...
    is_accettato = False
    for sa in stati_accettati:
        # modified since now sa contain dircetly the name onf the sate
        if sa == series_sequence[len(series_sequence)-1][0]:
            is_accettato = True
